...
- **Deleted:** `backend/migrate_add_hymnal.py` (after `0002` covers it). *Amendment 2026-09-26:* also `backend/migrate_add_hymn_facts.py` (PR #4) and its test `backend/tests/test_migrate_hymn_facts.py`, after `0001` and `0002` cover its columns. `backend/backfill_hymn_facts.py` and `backend/hymnary_facts.py` **stay**: the backfill is an ops CLI that fills data, not schema. In `README.md` → "Service rubric: hymn year and familiarity", step 1 (run `migrate_add_hymn_facts.py`) is replaced by "The columns come from the Alembic migrations (`alembic upgrade head`).", and the backfill steps are unchanged.
...
|---|---|---|---|
| `0001_baseline` | Creates the 11 tables exactly as `db/models.py` at `50fda65` (users, churches, memberships, invites, hymn_catalog, hymns, services, hymn_usage, contacts, gmail_tokens, oauth_states), including `ix_hymns_church_hymnal`. **Amendment 2026-09-26:** the reference is now `db/models.py` on `main` after PR #4 (`291aae9` or later; the models are unchanged since). That adds the nullable `text_year INTEGER` and `hymnal_count INTEGER` columns to **both** `hymns` and `hymn_catalog`. They are already in production (applied by PR #4's `migrate_add_hymn_facts.py` before it merged), so stamping at this baseline is correct. | Drops them | Fresh databases only (CI, new local dev). Production is **stamped**. |
| `0002_reconcile` | `op.create_index("ix_hymns_church_hymnal", "hymns", ["church_id", "hymnal"], if_not_exists=True)` plus any other difference the runbook's drift script reports, each guarded with `IF NOT EXISTS`. **Amendment 2026-09-26:** also, for `hymns` and `hymn_catalog`, add `text_year` and `hymnal_count` (INTEGER, nullable) only where missing. On Postgres this is `ALTER TABLE … ADD COLUMN IF NOT EXISTS …`, which also renders in offline `--sql` mode. On SQLite, which has no `IF NOT EXISTS` for columns, it uses the inspector check that `migrate_add_hymn_facts.py` uses. It is a no-op on production and on fresh databases. It fixes a local database made by `create_all` before PR #4 and then stamped at `0001`. | **No-op** for every object the `0001` baseline declares (including `ix_hymns_church_hymnal`), with a comment: on a fresh database 0001 created them and 0001's downgrade drops them, so dropping them here would break 0001's own `drop_index` and the `downgrade base` cycle. Only a drift fix that is not part of the 0001 model schema gets a guarded `drop_…(if_exists=True)` (none expected). | No data touched. `migrate_add_hymnal.py` deleted in the same PR. |
...
5. `alembic upgrade 0001_baseline:head --sql > upgrade.sql` and read it. (Offline mode cannot read the database, so without the explicit start revision it would render from base, including every 0001 `CREATE TABLE` and the `alembic_version` DDL.) Expect only the 0002 index, the 0003 `DO` block and `REVOKE`/`ALTER DEFAULT PRIVILEGES` statements, the 0004 columns, FK and `UPDATE`, and the `UPDATE alembic_version` lines. *Amendment 2026-09-26:* also expect 0002's four `ADD COLUMN IF NOT EXISTS` statements for `text_year` and `hymnal_count` on `hymns` and `hymn_catalog`. They are no-ops on production.
6. `python scripts/schema_drift.py` (`alembic check` refuses here with "Target database is not up to date." because the database is at 0001 and head is 0004). Expected diff, exactly: `add_column invites.reusable`, `add_column invites.accepted_by`, `add_fk fk_invites_accepted_by_users`, and `add_index ix_hymns_church_hymnal` only if it is missing. *Amendment 2026-09-26:* `text_year` and `hymnal_count` must **not** appear, because they are already in production. If they do, the database is not the one PR #4's migration ran against: stop and check `DATABASE_URL` with the owner before merging. `0002` would add them, but a production that lacks them is also failing the Streamlit app. Any other difference: if it is expand-safe (F§3.4), add it to `0002` with an `IF NOT EXISTS` guard and repeat from 5; otherwise stop and resolve with the owner before merging.
...
**Local dev** (README): fresh database → `cd backend && alembic upgrade head`; a database made by the old `create_all` → `alembic stamp 0001_baseline && alembic upgrade head`. Tests keep `create_all` in fixtures. *Amendment 2026-09-26:* a local database made before PR #4, which lacks `text_year`/`hymnal_count`, gets them from `0002`'s guarded add; `migrate_add_hymn_facts.py` is no longer needed.
...
- *Amendment 2026-09-26* (`test_migrations.py`): a SQLite database with the pre-PR-#4 schema (`hymns` and `hymn_catalog` without `text_year`/`hymnal_count`), stamped at `0001_baseline`, then `upgrade head` → both columns exist on both tables and `schema_diff` is empty. Running `upgrade head` again changes nothing. On a fresh `upgrade head`, `0001` creates the columns and `0002` adds nothing. `alembic upgrade 0001_baseline:head --sql` on Postgres renders the four `ADD COLUMN IF NOT EXISTS` statements. `backend/tests/test_migrate_hymn_facts.py` is deleted with the script.
...
18. *(Amendment 2026-09-26, PR #4.)* `0001_baseline` includes `text_year` and `hymnal_count` on `hymns` and `hymn_catalog`. Production's step 6 drift does not list them. A pre-PR-#4 local database gets them from `0002`'s guarded add. `backend/migrate_add_hymn_facts.py` and its test are deleted with `migrate_add_hymnal.py`, while `backfill_hymn_facts.py` stays as an ops CLI, and the README says so. *(test + deployed)*
