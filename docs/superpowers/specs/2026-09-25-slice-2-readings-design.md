...
class ReadingSetOut(BaseModel):
    name: str                                   # "Liturgy of the Passion", "Nineteenth Sunday after Pentecost", "Ash Wednesday"
                                                # (a Vanderbilt "Proper N (M)" row is renamed for a Sunday; see "Ordinary-time occasion names")
...
- `sunday_name(d)` is date-only (it no longer depends on the season string) and returns `None` for any non-Sunday. **This fixes a weekday such as Ascension Thursday being labelled "Sixth Sunday of Easter".** It covers: Advent 1–4; Baptism of the Lord; Second–Ninth Sunday after the Epiphany; Transfiguration; Lent 1–5; Palm Sunday; Easter Sunday; Easter 2–7; **Day of Pentecost**; **Trinity Sunday**; every **Nth Sunday after Pentecost** from the Second onward; **All Saints Day** when Nov 1 falls on a Sunday; **Reign of Christ** (the Sunday before Advent 1). The existing wording stays ("… Sunday in Lent", "… Sunday of Advent"); "after Epiphany" gains "the" (owner decision, see "Ordinary-time occasion names" below).
...
**Ordinary-time occasion names (owner decision A; resolves index open question 4).** Two defaults changed:
- **After Pentecost:** `pentecost = easter_date(y) + 49 days`; for a Sunday `d >= pentecost`, `n = (d - pentecost).days // 7`. `n == 0` is "Day of Pentecost" (unchanged); otherwise the name is **"{Ordinal} Sunday after Pentecost"**, spelled out in words ("Nineteenth", "Twenty-Third"). Trinity Sunday is `n == 1` but keeps its own name — named days always win over the computed ordinal.
- **After the Epiphany:** unchanged in shape, only the wording gains "the": **"{Ordinal} Sunday after the Epiphany"**. The Baptism of the Lord (`n == 0`) and Transfiguration Sunday (the last Sunday before Lent) keep their names.
- **Named days that always win**, checked before either computation: Trinity Sunday, All Saints Day (when Nov 1 is a Sunday), Christ the King / Reign of Christ (the Sunday before Advent 1), Baptism of the Lord, Transfiguration Sunday. World Communion Sunday is **not** a lectionary name and is not added.
- **Vanderbilt row rename.** A Vanderbilt row's raw `Liturgical Date` text becomes `ReadingSet.name` unchanged, **except** when it matches `Proper \d+ \(\d+\)` on a Sunday: then the name is `sunday_name(d)` instead (every such Sunday is covered by one of the two computations or a named day above, so this never falls back to the raw text). This runs inside `vanderbilt_sets_on`, before `fits_draft_limits` and `merge` see the sets, so the set switcher, the "From the Revised Common Lectionary: {set name}" caption and the auto-filled Occasion field all show the descriptive name. A row Vanderbilt already names specifically (`"Trinity Sunday"`, `"Visitation"`, `"All Saints Day"`, …) is left exactly as written. The RCL Proper number keeps deciding nothing about this rename: `merge` still matches Lectio groups to Vanderbilt sets by `scripture_key(gospel)`, never by name, and the Proper number is never shown.
- **Fixture**, computed with `easter_date` rather than hand-picked (also table-driven tests in `test_lectionary_domain.py`):

  | Date | `sunday_name(d)` | Why |
  |---|---|---|
  | 2026-05-24 | Day of Pentecost | `n = 0` |
  | 2026-05-31 | Trinity Sunday | `n = 1`; named day wins over "First Sunday after Pentecost" |
  | 2026-06-07 | Second Sunday after Pentecost | `n = 2` |
  | 2026-10-04 | Nineteenth Sunday after Pentecost | `n = 19` |
  | 2026-11-01 | All Saints Day | `n = 23`; named day wins over "Twenty-Third Sunday after Pentecost" |
  | 2026-11-22 | Reign of Christ | last Sunday before Advent 1 (2026-11-29); `n = 26`, overridden |
  | 2027-11-21 | Reign of Christ | **year-boundary/Advent edge case:** Advent 1 2027 is 2027-11-28 (the same boundary date `liturgical_year_for` tests); `n = 27` here, one higher than 2026's 26, because Easter 2027 (March 28) falls earlier — the ordinal is computed fresh per year, never looked up from the Proper number |
  | 2025-11-23 | Reign of Christ | another year-boundary case: `n = 24`; Advent 1 2025 is 2025-11-30 |
  | 2026-01-11 | Baptism of the Lord | first Sunday after Epiphany (Jan 6, 2026 is a Tuesday) |
  | 2026-01-18 | Second Sunday after the Epiphany | wording now includes "the" |
  | 2026-02-15 | Transfiguration Sunday | last Sunday before Ash Wednesday (2026-02-18); named day wins over "Sixth Sunday after the Epiphany" |
...
|---|---|---|---|
| 2026-10-04 (Sun) | Proper 22 (27), two-track cells | Matthew 21:33-46 | [**Nineteenth Sunday after Pentecost** ← Lectio readings ★] (Vanderbilt's raw name "Proper 22 (27)" is renamed by the ordinary-time naming rule; the Proper number still matches the Lectio group). Today: "Ordinary Time — Year A". |
...
| 2026-11-01 (Sun) | All Saints Day; Proper 26 (31) | Matthew 23:1-12 | [All Saints Day, **Twenty-Third Sunday after Pentecost** ← Lectio ★] (the "Proper 26 (31)" row is renamed; "All Saints Day" is left as Vanderbilt wrote it) |
...
8. **Occasion names.**
   - Ordinary Sundays show a descriptive computed name ("Nineteenth Sunday after Pentecost", "Second Sunday after the Epiphany") instead of "Ordinary Time — Year A" or the RCL designation ("Proper 22 (27)"); the Proper number stays internal to lectionary matching (owner decision A).
   - Weekday feasts get real names ("Ash Wednesday", "Good Friday").
   - Computed fallback names gain Pentecost, Trinity, All Saints (on a Sunday) and Reign of Christ, and never give a Sunday name to a weekday.
...
4. `liturgical_year_for(2027-11-28) == "2027-28"` and `liturgical_year_for(2026-11-28) == "2025-26"`. `sunday_name` returns None for every non-Sunday, computes "{Ordinal} Sunday after Pentecost" / "after the Epiphany" from `easter_date`, and gives a named day (Trinity, All Saints on a Sunday, Reign of Christ, Baptism of the Lord, Transfiguration) precedence over the computed ordinal in every fixture year (2025–2028). A Vanderbilt "Proper N (M)" row name is replaced by `sunday_name(d)`; a specifically named Vanderbilt row is not. *(test)*
...
1. ~~Occasion names for ordinary-time Sundays.~~ **Decided (owner decision A).** Ordinary-time Sunday names are computed descriptively — "Nth Sunday after Pentecost" and "Nth Sunday after the Epiphany" — never the RCL "Proper N (M)" designation, with named days (Trinity Sunday, All Saints on a Sunday, Christ the King / Reign of Christ, Baptism of the Lord, Transfiguration) always winning. See "Ordinary-time occasion names" above. The field is still always editable, and the Proper number still drives Lectio/Vanderbilt matching internally.
