...
20. **Rubric behavior (amendment 2026-09-26; compared with Streamlit after PR #4, inv D10):**
    ...
    - Changed: the cut keeps 10 of 50 places for newer and unknown-year hymns (Streamlit: 12 of 60; the same one-in-five share, owner-approved 2026-09-26). A hymnal with no facts is sampled evenly instead of cut at its first hymns, and a small focused list is padded with an even sample of the rest of the hymnal instead of the whole ranked hymnal. Both fix the PH1990 Advent bias that PR #4 kept (inv D7). The pad sample is ranked by the rubric before it is appended, so older and familiar hymns still come first within it, as in Streamlit; with no facts it is the plain even sample.
